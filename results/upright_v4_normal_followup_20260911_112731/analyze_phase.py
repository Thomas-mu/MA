...
ROOT=BASE.parents[1]
PHASES=('standstill','normal_before','airflow_modified','normal_after','normal_followup')
...
def analyze(phase):
    ...
    assert s['mounting_id']==json.loads((BASE/'mounting.json').read_text())['mounting_id']
    assert meta['mounting_id']==s['mounting_id']
    still=phase=='standstill';duration=30 if still else 300
    origin=s['record_invoked_monotonic_ns'] if still else s['command_invocation_monotonic_ns'];t=(ns-origin)/1e9
    ...
    late=blocks if still else blocks[36:60]
    y=np.array([b['vector_ac_rms_mg'] for b in late]);x=np.array([b['start_s']+2.5 for b in late])
    slope,intercept=np.polyfit(x,y,1)
    robust=np.median([(y[j]-y[i])/(x[j]-x[i]) for i in range(len(y)) for j in range(i+1,len(y))])
    primary={**describe(y),'windows':len(y),'linear_slope_mg_min':float(slope*60),
        ...
        'last60_minus_first60_mg':None if still else float(y[12:].mean()-y[:12].mean()),
        'four_30s_means_mg':None if still else [float(y[i:i+6].mean()) for i in range(0,24,6)],
    ...
    quality={'xyz_points':len(d),'single_axis_values':3*len(d),'nominal_odr_hz':odr,
        ...
        **{k+'_count':int(d[k].sum()) for k in ('gap','overrun','saturated')},
        'points_after_analysis_duration':int((t>=duration).sum()),
        'analysis_time_origin':'record invocation' if still else 'PWM command invocation',
    ...
    assert commands==([] if still else [75.0,0.0])
    assert s['final_readback']['pwm_configuration']['configured_duty_percent']==0
    release_path=BASE/'initial_release.json' if phase in ('standstill','normal_before') else BASE/f'{phase}_release.json'
    paths=[session_path,csv,sidecar,BASE/f'{phase}_fan.jsonl',release_path,BASE/'mounting.json']
    report={'created_utc':datetime.now(timezone.utc).isoformat(),'phase':phase,
        'csv':s['csv'],'mounting_id':s['mounting_id'],'geometry':s.get('geometry'),
        'sensor':meta['sensor'],'quality':quality,'primary_180_300':None if still else primary,
        'all_window_summary':describe([b['vector_ac_rms_mg'] for b in blocks]),
        'timing':s['timing'],'additional_off_from_release_actual_s':s.get('additional_off_from_release_actual_s'),
        'total_off_since_previous_zero_s':s.get('total_off_since_previous_zero_s'),
        'command_invocation_utc':s.get('command_invocation_utc'),'zero_command_completed_utc':s.get('zero_command_completed_utc'),
        'source_sha256':digest(Path(__file__)),'input_sha256':{str(p.relative_to(ROOT)):digest(p) for p in paths},
        'control_transactions':commands,'final_pwm_percent':0,'mechanical_standstill_user_confirmed_before_capture':True if still else None,
...
def plot(phase,blocks,primary,t,dt,out):
    ...
    if phase=='standstill':
        ...
        ax[0].plot([b['start_s']+2.5 for b in blocks],[b['vector_ac_rms_mg'] for b in blocks],'o-')
        ax[0].set(ylabel='Vektor-AC-RMS [mg]',title='Stillstandsreferenz: eigene Achsenmittelwerte je 5-s-Fenster entfernt')
        ax[1].plot(t,dt*1000,'.',markersize=1,rasterized=True)
        ax[1].set(ylabel='Host-Leseabstand [ms]')
        for a in ax:a.set(xlim=(0,30),xlabel='Zeit seit Aufruf der Erfassung [s]');a.grid(alpha=.25)
        fig.suptitle('Aufrechte Aufstellung: bestätigter Stillstand, 0 % PWM')
    ...
    fig.suptitle(f'Aufrechte Aufstellung — {phase}: 75 % PWM, 25 kHz')
    fig.savefig(out/'phase_overview.png',dpi=160);fig.savefig(out/'phase_overview.pdf');plt.close(fig)
