    ...
    try:
        with FanPWM(journal_path=journal) as fan:
            try:
                ...
                with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                    ...
                    if session['sensor'] != previous['sensor']:
                        raise ValueError('Sensor configuration differs from preceding v4 reference')
        ...
        if 'command_invocation_monotonic_ns' in session:
            ...
            if previous and 'zero_command_completed_monotonic_ns' in previous:
                ...
                session['total_off_time_source'] = 'same-boot monotonic times; preceding zero from parent v4 sequence'
            else:
        ...
        persist(path, session)
    emit('phase_finished', phase=args.phase, status=session['status'], next_action='completed_no_further_start',
