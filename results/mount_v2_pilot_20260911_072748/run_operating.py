"""One300-s startup for mountingv2; explicit full safety gate and no reply deadline."""
...
from pilot_common import ROOT, BASE, MOUNTING_ID, require_safety, require_standstill, mounting_metadata
...
def main():
    ...
    confirmation_path = BASE / 'before_standstill_confirmation.json'
    confirmation = require_standstill('before')
    initial_references_path = BASE / 'before_session.json'
    initial_references = json.loads(initial_references_path.read_text())
    if initial_references.get('status') != 'completed' or len(initial_references.get('recordings', [])) != 2:
        raise ValueError('Two completed initial standstill references required before the single start')
    if initial_references.get('mounting_id') != MOUNTING_ID:
        raise ValueError('Initial references use another mounting version')
    ...
    session = {
        ...
        'safety_confirmation_sha256': hashlib.sha256((BASE / 'safety_confirmation.json').read_bytes()).hexdigest(),
        'common_source_sha256': hashlib.sha256((BASE / 'pilot_common.py').read_bytes()).hexdigest(),
        'initial_references_sha256': hashlib.sha256(initial_references_path.read_bytes()).hexdigest(),
