...
def verify(output):
    ...
    for source in plan["sources"]:
        ...
        boot_ids.append(meta["user_release"]["boot_id"])
    ...
    for split in expected:
        for kind in ("raw_g", "ac_g"):
            ...
            check(actual.dtype == np.float32, "Expected Float32 windows")
        ...
        check(labels.dtype == np.int8, "Expected Int8 labels")
...
def main():
    ...
    if args.verify:
        if args.output:
            parser.error("--output is not used with --verify")
