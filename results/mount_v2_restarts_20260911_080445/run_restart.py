"""One explicitly selected new-mount restart: fixed confirmed off time, then300s, then0%."""
...
from restart_common import ROOT, BASE, MOUNTING_ID, PLAN, prepare_run, mounting_metadata
...
def main():
    ...
    safety, confirmation, previous, previous_path = prepare_run(run)
    confirmation_path = BASE / f'run{run}_standstill_confirmation.json'
    ...
    session_path, journal = BASE / f'run{run}_session.json', BASE / f'run{run}_fan.jsonl'
    ...
    csv_path = Path('data') / BASE.name / f'normal75_run{run}_300s_{stamp}.csv'
    ...
    session = {
        ...
        'safety_confirmation_sha256': hashlib.sha256((ROOT / PLAN['safety_source']).read_bytes()).hexdigest(),
        'common_source_sha256': hashlib.sha256((BASE / 'restart_common.py').read_bytes()).hexdigest(),
        'previous_shutdown_source': str(previous_path.relative_to(ROOT)),
        'previous_shutdown_sha256': hashlib.sha256(previous_path.read_bytes()).hexdigest(),
        'run': run, 'planned_controlled_off_s': 60,
            ...
            with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                ...
                metadata = {
                    **mounting_metadata(),
                    ...
                    'purpose': 'pilot', 'sequence_id': BASE.name, 'run': run,
                    'primary_comparison_interval_s': [180,300], 'settling_time_validated': False,
                ...
                persist(session_path, session)
                target = confirmation['accepted_monotonic_ns'] + 60_000_000_000
                session['off_timer_target_monotonic_ns'] = target
                persist(session_path, session)
                emit('controlled_off_wait', run=run, target_seconds_after_confirmation=60,
                ...
                session['off_end_readback'] = fan.verify(0)
                ...
                session['controlled_off_actual_to_command_invocation_s'] = (session['command_invocation_monotonic_ns'] - confirmation['accepted_monotonic_ns'])/1e9
                session['controlled_off_actual_to_command_completion_s'] = (session['command_completed_monotonic_ns'] - confirmation['accepted_monotonic_ns'])/1e9
                session['total_off_since_previous_zero_to_command_s'] = (datetime.fromisoformat(session['command_invocation_utc'])-datetime.fromisoformat(previous['zero_command_completed_utc'])).total_seconds()
                metadata.update(
