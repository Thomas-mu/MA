"""One confirmed restart, fixed 60-s settling, 30-s pilot, then explicit zero.
...
def main():
    ...
    trial = args.trial
    confirmation_path = BASE / f'standstill_before_start_{trial}.json'
    ...
    if trial > 1:
        previous = json.loads((BASE / f'start_{trial - 1}.json').read_text())
    ...
    running_path = BASE / f'running_start_{trial}.json'
    session_path = BASE / f'start_{trial}.json'
    ...
    with session_path.open('x') as handle:
        handle.write('{}\n')
    journal = BASE / f'start_{trial}_fan.jsonl'
    session = {
        'trial': trial, 'started_utc': utc(), 'status': 'preflight',
    ...
    with FanPWM(journal_path=journal) as fan:
        try:
            ...
            with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                ...
                metadata = {
                    'purpose': 'pilot', 'sequence_id': BASE.name, 'restart_trial': trial,
