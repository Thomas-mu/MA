"""One confirmed restart; fixed settling/capture without a timed chat-response gate.

Run only after the agent has announced 75%, the timed capture and automatic
return to0%. A new visual standstill confirmation is required for every start. Running observations are stored separately after capture; they are never presumed.
    ...
    with FanPWM(journal_path=journal) as fan:
        try:
            ...
            with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                ...
                session['status'] = 'settling_fixed_timer_running_observation_pending'
                ...
                session['running_observation_status'] = 'pending_separate_operator_report_after_capture'
                ...
                metadata = {
                    ...
                    'physical_state_source': 'prestart_standstill_confirmed; running_observation_pending_separate_postcapture_report',
                    'standstill_confirmation': confirmation, 'operator_confirmation': None,
                    'running_observation_confirmed_at_capture': False,
                    'protocol_revision': 'fixed_timer_without_timed_chat_response_gate',
                ...
                df = record(bus, RECORD_S, 200, output_path=csv_path, label=-1,
                            state='operating75_restart_pilot_visual_observation_pending', metadata=metadata)
