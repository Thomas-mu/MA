...
def mark(run, event, number, note):
    ...
    with (run / 'cues.jsonl').open('a+', encoding='utf-8') as handle:
        ...
        if event == 'call_requested' and any(r['event'] == event for r in previous):
            raise ValueError('Nur ein Anruf pro Methodenlauf ist vorgesehen.')
        if event == 'end_reported' and not any(r['event'] == 'call_requested' for r in previous):
            raise ValueError('Noch keine Anrufaufforderung protokolliert.')
...
def capture(plan_directory, method, output, seconds, *, technical_check=False):
    ...
    shutil.copy2(__file__, output / 'capture_source.py')
...
def render_report(run):
    ...
    axes[2].plot(x, [float(r['process_cpu_percent_one_core']) for r in rows])
    axes[2].set_ylabel('CPU [% eines Kerns]')
    axes[3].plot(x, [float(r['process_rss_bytes'])/2**20 for r in rows])
    axes[3].set_ylabel('RAM [MiB]'); axes[3].set_xlabel('Zeit seit Aufnahmestart [s]')
    for axis in axes:
        axis.grid(alpha=.25)
        for cue in calls:
