...
def html_slide_export(pptx: Path, output: Path) -> Path:
    ...
    parts = [f'''<!doctype html><html lang="de"><meta charset="utf-8"><title>Vorbereitung</title>
...
def export_pdf_and_previews(pptx: Path, output: Path) -> None:
    ...
    if completed.returncode != 0 or "source file could not be loaded" in completed.stderr or not pdf.is_file():
    ...
    if len(images) != 10:
        raise ValueError(f"Expected ten slide previews; found {len(images)}")
    contact = Image.new("RGB", (1600, 5 * 450), "#DCE6EC")
