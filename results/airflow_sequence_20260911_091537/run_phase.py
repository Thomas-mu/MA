...
def main():
    ...
    session = dict(status='preflight', started_utc=utc(), phase=args.phase,
        ...
        mounting_id=MOUNTING_ID, geometry=geometry,
    ...
    try:
            ...
            try:
                ...
                with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                    ...
                    metadata = dict(mounting_id=MOUNTING_ID, geometry=geometry,
                        mounting='two screws on stationary outer frame; board oblique; user instruction to keep unchanged',
        ...
        if 'command_invocation_monotonic_ns' in session:
            ...
            if previous:
                ...
                session['total_off_time_source'] = 'no preceding run in this sequence; no continuously verified earlier off interval'
