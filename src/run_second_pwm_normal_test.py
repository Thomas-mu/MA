#!/usr/bin/env python3
"""One explicitly released independent normal capture; never loop or retry.
...
def validate_release(protocol, release, previous, *, boot_id, now_ns):
    ...
    check(release.get("released") is True, "Explicit release required")
    required_readiness = readiness_fields(protocol, release)
    ...
    for field, value in required_readiness.items():
        check(field in release and release[field] is value, f"Readiness missing: {field}")
    ...
    if index == 0:
        ...
        check(previous.get("boot_id") == boot_id and previous.get("protocol_id") == protocol["protocol_id"],
              "Previous run belongs to another boot/protocol")
...
def execute_capture(fan, bus, session, csv_path, metadata, recorder=None):
    ...
    try:
        ...
        session["setting_result"] = fan.set_percent(50)
        ...
        emit("capture_start", phase=session["phase"], pwm_percent=50, frequency_hz=25000, duration_s=300, csv=str(csv_path))
        df = recorder(bus, 300, 200, output_path=csv_path, label=0, state="normal", metadata=metadata)
        session["recording"] = df.attrs["report"]
        session["after_capture_readback"] = fan.verify(50)
...
def release_gate(protocol_path, phase, release_path):
    ...
    check(protocol["purpose"] == "independent_second_normal_pwm_test" and protocol["planned_runs"] == PHASES, "Wrong protocol")
    check(protocol["pwm_percent"] == 50 and protocol["pwm_frequency_hz"] == 25000 and protocol["duration_s"] == 300,
    ...
    validate_release(protocol, release, previous, boot_id=Path("/proc/sys/kernel/random/boot_id").read_text().strip(), now_ns=time.monotonic_ns())
...
def main():
    ...
    # Validate the full 50-percent data contract before importing any hardware.
    ...
    try:
        ...
        with FanPWM(journal_path=journal) as fan:
            try:
                ...
                with connect(odr_hz=200, range_g=2, fifo=True) as bus:
                    ...
                    csv_path = Path(protocol["data_directory"]) / f"{args.phase}_50pwm_300s_{stamp}.csv"
                    ...
                    metadata = {"purpose": "test", "split": "independent_test", "phase": args.phase,
                        "condition_label": "normal", "protocol_id": protocol["protocol_id"],
                        ...
                        "user_release": release, "fan_pwm_setpoint_percent": 50, "fan_pwm_frequency_hz": 25000,
                        ...
                        "no_plate_user_confirmed": True, "running_observation_requested": False}
        ...
        if "command_invocation_monotonic_ns" in session:
            ...
            session["total_off_since_previous_zero_s"] = (command-previous["zero_command_completed_monotonic_ns"])/1e9 if previous else None
            session["total_off_time_note"] = "Software zero-completion to start-command interval; exact mechanical stop/cooling interval unknown. First run total previous off-time unknown."
