...
def load_recording(path: Path, *, label: int, state: str) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    times, samples, indices = [], [], []
    quality_columns = {"gap", "overrun", "saturated"}
    quality_counts = {key: 0 for key in sorted(quality_columns)}
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        available_quality = quality_columns.intersection(reader.fieldnames)
        if available_quality and available_quality != quality_columns:
            raise ValueError(f"{path}: unvollständige Messqualitätsflags.")
        for index, row in enumerate(reader):
            ...
            for key in available_quality:
                flag = row[key].strip().lower()
                if flag not in ("true", "false", "1", "0"):
                    raise ValueError(f"{path}:{index + 2}: ungültiges Qualitätsflag {key}.")
                quality_counts[key] += int(flag in ("true", "1"))
            if "sample_index" in row:
                indices.append(int(row["sample_index"]))
    if any(quality_counts.values()):
        raise ValueError(f"{path}: Aufnahme wegen gap/overrun/saturated zurückgewiesen: {quality_counts}")
    if indices and np.any(np.diff(indices) != 1):
        raise ValueError(f"{path}: nicht fortlaufende Sample-Indizes; Aufnahme zurückgewiesen.")
    ...
    report = {"path": str(path.resolve()), "sha256": sha256(path),
              ...
              "quality_flags_present": bool(available_quality),
              "quality_flag_counts": quality_counts if available_quality else None,
              "quality_note": "Fehlende Flags bedeuten unbekannt; keine nachträgliche Frischwertgarantie.",
              "timestamp_note": "Zeitstempelrate belegt allein keine neuen Sensorwerte."}
    return values, timestamps, report


def acquisition_sidecar(csv_path: Path, report: dict, *, purpose: str,
                        expected_hash: str | None = None) -> dict:
    """Prüft das journalisierte Aufnahmeprotokoll zusätzlich zu Benutzerangaben."""
    path = csv_path.with_suffix(".json")
    if not path.exists() or not report["quality_flags_present"]:
        raise ValueError(f"{csv_path}: finales Aufnahmeprotokoll/Qualitätsflags fehlen.")
    digest = sha256(path)
    if expected_hash is not None and digest != expected_hash:
        raise ValueError(f"{path}: Sidecar-Hash stimmt nicht mit Testmanifest überein.")
    document = read_json(path)
    if document.get("csv_sha256") != report["sha256"] or document.get("status") != "completed":
        raise ValueError(f"{path}: Aufnahmehash oder abgeschlossener Status unbestätigt.")
    if document.get("purpose") != purpose or document.get("label") != report["label"]:
        raise ValueError(f"{path}: Aufnahmezweck/Label widersprechen dem Split.")
    if document.get("state") != report["state"]:
        raise ValueError(f"{path}: Zustandskennzeichnung widerspricht Aufnahmeprotokoll.")
    if document.get("mounting") in (None, "", "not_documented"):
        raise ValueError(f"{path}: Montagebeschreibung fehlt.")
    sensor = document.get("sensor", {})
    if sensor.get("acquisition_mode") != "fifo_stream" or not sensor.get("register_readback"):
        raise ValueError(f"{path}: FIFO/ausgelesene Sensorregister unbestätigt.")
    if not sensor.get("odr_hz") or not sensor.get("range_g"):
        raise ValueError(f"{path}: Sensor-ODR/Messbereich fehlen.")
    if document.get("detection_controls_fan") is not False:
        raise ValueError(f"{path}: deaktivierte detektionsabhängige Lüftersteuerung unbestätigt.")
    report["acquisition_sidecar"] = {"path": str(path), "sha256": digest,
                                     "sensor": sensor, "mounting": document["mounting"]}
    return document
...
def binary_metrics(labels: list[int], predictions: list[int]) -> dict:
    ...
    return {"windows": len(true), "normal_windows": tn + fp, "anomaly_windows": tp + fn,
            ...
            "undefined_metric_representation": "null (Nenner 0)"}
...
def calibrate(args) -> None:
    ...
    chain_verified = chain_verified and all(s["mounting"] == mounting for s in sidecars)
    bundle = {"schema_version": 1, "profile_name": profile["profile_name"],
              ...
              "sensor": sensor, "mounting": mounting, "scaler": scaler, "recordings": reports,
...
def test_data(manifest_path: Path, bundle: dict, bundle_path: Path, pilot: bool):
    ...
    if not pilot:
        ...
        if not protocol.get("path") or not protocol.get("sha256"):
            raise ValueError("Finaler Test benötigt ein separates, gehashtes Versuchsprotokoll.")
        if sha256(resolve_source(protocol["path"], manifest_path.parent)) != protocol["sha256"]:
            raise ValueError("Versuchsprotokoll-Hash widerspricht dem Testmanifest.")
    ...
    for entry in manifest["recordings"]:
        ...
        if not pilot:
            if not entry.get("metadata_sha256"):
                raise ValueError("Finale Testaufnahme benötigt metadata_sha256 des JSON-Aufnahmeprotokolls.")
            capture = acquisition_sidecar(path, report, purpose="test", expected_hash=entry["metadata_sha256"])
            for key in ("odr_hz", "range_g", "acquisition_mode", "register_readback"):
                if capture["sensor"].get(key) != bundle["sensor"].get(key):
                    raise ValueError(f"Aufnahmeprotokoll und eingefrorene Messkette unterscheiden sich: {key}.")
            if capture["mounting"] != bundle.get("mounting"):
                raise ValueError("Sensor-Montage stimmt nicht mit der eingefrorenen Kalibrierung überein.")
            if capture.get("fan_pwm_setpoint_percent") != pwm or capture.get("rpm_measured") != rpm:
                raise ValueError("PWM-Vorgabe oder gemessene Drehzahl widerspricht Aufnahmeprotokoll.")
...
def evaluate(args) -> None:
    ...
    write_json(output / "quality.json", {
        ...
        "comparison_complete_on_all_windows": not any(r["decision"] == "INVALID" for r in rows),
        "limitations": bundle["limitations"] + [
            ...
            "Bei INVALID ist der gemeinsame Gütevergleich unvollständig; Kennzahlen gültiger Teilmengen sind kein gleichwertiger Vergleich.",
...
class TimingJournal:
    """Fester Schreibpuffer; exakte Quantile erst nach der Ressourcenmessung."""
    def __init__(self, path: Path):
        self.path, self.count, self.buffer_bytes = path, 0, 64 * 1024
        self.handle = path.open("xb", buffering=self.buffer_bytes)

    def add(self, milliseconds):
        if not math.isfinite(milliseconds) or milliseconds < 0:
            raise ValueError("Latenz muss endlich und nichtnegativ sein.")
        self.handle.write(struct.pack("<d", milliseconds))
        self.count += 1

    def close(self):
        self.handle.close()

    def read_after_measurement(self):
        if not self.handle.closed:
            raise ValueError("Latenzjournal erst nach Messende laden.")
        return np.fromfile(self.path, dtype="<f8")
...
def benchmark_worker(args) -> None:
    ...
    output = resolve_source(args.output)
    timings = TimingJournal(output / f"{args.method}_latencies.f64le")
    cpu_start, wall_start = process.cpu_times(), time.perf_counter_ns()
    ...
    with ResourceSampler(process) as resources:
        try:
            ...
            while (timings.count < args.windows or
                ...
                for window in raw:
                    ...
                    timings.add((available - ready) / 1e6)
        finally:
            timings.close()
    wall_s = (time.perf_counter_ns() - wall_start) / 1e9
    cpu_end = process.cpu_times()
    cpu_s = (cpu_end.user - cpu_start.user) + (cpu_end.system - cpu_start.system)
    # Zusätzlicher Speicher der exakten Statistik/CSV-Ausgabe liegt strikt nach
    # dem Ende von Prozess-CPU- und RSS-Messung.
    durations = timings.read_after_measurement()
    result = {"method": args.method, "runtime": runtime, "provenance": provenance(),
              ...
              "warmup_windows": args.warmup, "measured_windows": timings.count,
              "minimum_requested_windows": args.windows,
              "minimum_requested_seconds": args.minimum_seconds,
              "complete_passes_over_same_windows": timings.count // len(raw),
              ...
              "latency_ms": {"mean": float(np.mean(durations)), "p50": float(np.median(durations)),
                             "p95": float(np.percentile(durations, 95)), "p99": p99(durations),
                             "min": float(durations.min()), "max": float(durations.max())},
              "latency_sample_count": len(durations),
              "latency_write_buffer_bytes": timings.buffer_bytes,
              "latency_percentiles_are_sample_estimates": False,
              "latency_statistics_note": "Alle Latenzen in Little-Endian-Float64-Journal mit festem64KiB-Schreibpuffer; Schreiben außerhalb Verarbeitungslatenz, innerhalb Prozess-CPU. Exakte empirische Quantile/CSV-Ausgabe erst nach Ende der CPU/RSS-Messung.",
              "postmeasurement_statistics_array_bytes": durations.nbytes,
    ...
    with (output / f"{args.method}_latencies.csv").open("x", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["iteration", "source_window", "processing_latency_ms"])
        writer.writerows((i, i % len(raw), d) for i, d in enumerate(durations))
