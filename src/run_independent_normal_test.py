...
PHASES = ["normal_test_01", "normal_test_02", "normal_test_03"]
...
def validate_release(protocol, release, previous, *, boot_id, now_ns):
    ...
    for field in ("ready_normal_no_plate", "external_supply_connected", "mechanical_standstill_confirmed", "mounting_unchanged"):
        check(release.get(field) is True, f"Readiness missing: {field}")
...
def release_gate(protocol_path, phase, release_path):
    ...
    check(protocol["purpose"] == "independent_normal_test" and protocol["planned_runs"] == PHASES, "Wrong protocol")
