...
class FrozenEngine:
    def __init__(self, bundle_directory, methods=None):
        ...
        if frozen_test.sha256(self.directory / 'pilot_bundle.json') != frozen_test.FROZEN_BUNDLE_SHA256:
            raise ValueError('Wrong authorized frozen package')
        self.methods = list(pilot.METHODS if methods is None else methods)
        if not self.methods or len(set(self.methods)) != len(self.methods) or not set(self.methods) <= set(pilot.METHODS):
            raise ValueError('Unknown or duplicated method')
        self.functions, self.runtimes = {}, {}
        for method in self.methods:
            self.functions[method], self.runtimes[method] = pilot.common.scorer(
                method, self.bundle, self.directory, runtime='auto', threads=1)

    def score(self, window):
        started = time.monotonic_ns()
        meta = window['metadata']
        shared = None
        if meta['quality_valid']:
            shared = pilot.standardize_ac(window['ac_float32'], self.bundle['scaler'])
        preprocessed = time.monotonic_ns()
        rows = []
        for method in self.methods:
            threshold = self.bundle['thresholds'][method]['value']
            row = dict(meta, method=method, threshold=threshold, score=None,
                       prediction=None, decision='INVALID', error=None)
            score_start = time.monotonic_ns()
            if shared is None:
                row['error'] = ', '.join(meta['quality_reasons'])
            else:
                try:
                    score = float(self.functions[method](shared.copy()))
                    label = pilot.common.finite_score(score, threshold)
                    row.update(score=score, prediction=label, decision='ANOMALY' if label else 'NORMAL')
                except Exception as exc:
                    row['error'] = f'{type(exc).__name__}: {exc}'
            finished = time.monotonic_ns()
            row.update(standardization_ns=preprocessed-started,
                       scorer_call_ns=finished-score_start,
                       decision_monotonic_ns=finished)
            # scorer_call includes tensor transfers/MSE; never call it invoke-only.
            rows.append(row)
        return rows
...
def run_stream(rows, command_ns, engine, output, *, capacity=4, paced=False,
    ...
    stats = dict(windows_dropped=0, queue_high_watermark=0, source_mode=source_mode,
                 ...
                 decision_interval_seconds=[180,300], warmup_invocations=0)
    resources = []
    metrics = ProcessMetrics()

    def monitor():
        while not metrics_done.wait(.1):
            resources.append(dict(monotonic_ns=time.monotonic_ns(), **metrics.sample()))
    ...
    try:
        ...
        resources.append(dict(monotonic_ns=time.monotonic_ns(), **metrics.sample()))
        pd.DataFrame(resources).to_csv(output/'resources.csv', index=False, mode='x')
        stats.update(assembler.summary())
        stats['decision_rows'] = len(decisions)
        ...
        stats['invalid_decisions'] = sum(r['decision']=='INVALID' for r in decisions)
