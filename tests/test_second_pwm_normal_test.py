...
def source(tmp_path, monkeypatch, synthetic_frame):
    ...
    p = {"schema_version": 1, "purpose": "independent_second_normal_pwm_test", "protocol_id": "independent_test_fixture",
         ...
         "sensor": copy.deepcopy(preparation.EXPECTED_SENSOR), "planned_runs": ["normal_pwm50_01", "normal_pwm50_02", "normal_pwm50_03"],
         "selection_seconds_since_pwm_command": [180, 300], "window_size": 128, "step_size": 128,
         "pwm_percent": 50, "pwm_frequency_hz": 25000, "duration_s": 300,
    ...
    m = {"schema_version": 1, "status": "completed", "recording_id": csv.stem,
         "phase": "normal_pwm50_01", "purpose": "test", "split": "independent_test", "state": "normal",
         "condition_label": "normal", "label": 0, "mounting_id": p["mounting_id"],
         "fan_pwm_setpoint_percent": 50, "fan_pwm_frequency_hz": 25000, "configured_duration_seconds": 300,
         ...
         "user_release": {"released": True, "phase": "normal_pwm50_01", "boot_id": "same-boot",
                          "source": "explicit_user_message", "authorized_phases": ["normal_pwm50_01"],
    ...
    journal = {"status": "completed", "phase": m["phase"], "mounting_id": m["mounting_id"], "boot_id": "same-boot",
               ...
               "after_capture_readback": {"pwm_configuration": {"period_ns": 40000, "duty_cycle_ns": 20000,
...
@pytest.mark.parametrize("field,value", [("purpose", "pilot"), ("split", "validation"), ("state", "normal_after"),
                                         ("fan_pwm_setpoint_percent", 75), ("mounting_id", "old"),
                                         ("configured_duration_seconds", 30), ("detection_controls_fan", True),
                                         ("phase", "normal_pwm50_04"), ("recording_id", "old"),
...
def test_import_rejects_corrupt_provenance_or_structural_time(source, kind):
    if kind == "csv_hash":
        ...
        source["journal"]["final_readback"]["pwm_configuration"]["duty_cycle_ns"] = 20000
        source["flush"]()
...
@pytest.mark.parametrize("field,value", [("ready_normal_no_plate", False), ("external_supply_connected", False),
                                         ...
                                         ("source", "inferred"), ("authorized_phases", ["normal_pwm50_02"]),
...
def test_protocol_validates_hash_fixed_parameters_code_and_training_exclusions(tmp_path, monkeypatch):
    ...
    p = {"schema_version": 1, "purpose": "independent_second_normal_pwm_test", "protocol_id": "fixture", "boot_id": "boot",
         ...
         "selection_seconds_since_pwm_command": [180, 300], "window_size": 128, "step_size": 128,
         "pwm_percent": 50, "pwm_frequency_hz": 25000, "duration_s": 300, "additional_off_after_release_s": 60,
    ...
    p["implementation_sha256"] = {str(q.resolve()): evaluation.sha256(q) for q in
        [Path(evaluation.__file__), Path(evaluation.__file__).with_name("independent_normal_test.py"),
         Path(evaluation.__file__).with_name("second_pwm_release.py")]}
