...
from src import run_independent_normal_test as runner
...
def release(phase="normal_test_01"):
    return {
...
def previous(phase="normal_test_01"):
    return {
        "phase": phase,
        "status": "completed",
        "boot_id": BOOT,
...
@pytest.mark.parametrize("phase,prior_phase", [
    ("normal_test_02", "normal_test_01"),
    ("normal_test_03", "normal_test_02"),
])
...
@pytest.mark.parametrize("field,value", [
    ...
    ("released", False),
    ("authorized_phases", ["normal_test_01", "normal_test_02"]),
    ("authorized_phases", ["normal_test_02"]),
...
def test_first_release_does_not_authorize_later_phase():
    with pytest.raises(ValueError):
        validate(release("normal_test_02"), None)
...
def test_predecessor_must_be_completed_same_study_and_stopped_before_release(field, value):
    ...
    prior[field] = value
    with pytest.raises(ValueError):
        validate(release("normal_test_02"), prior)
...
def capture_fixture(monkeypatch, tmp_path):
    ...
    calls = []
    bus = object()
    session = {"phase": "normal_test_01"}
    metadata = {}
    path = tmp_path / "normal_test_01.csv"
...
def gate_fixture(monkeypatch, tmp_path, phase="normal_test_01"):
    ...
    plan = protocol()
    plan.update(
        purpose="independent_normal_test", pwm_percent=75, pwm_frequency_hz=25000,
...
def test_release_gate_is_read_only_and_accepts_only_frozen_current_release(monkeypatch, tmp_path):
    ...
    plan, value, prior = runner.release_gate(plan_path, "normal_test_01", release_path)
    assert prior is None
    assert value["authorized_phases"] == ["normal_test_01"]
...
def test_any_previous_attempt_marker_blocks_replay_even_if_empty_or_failed(monkeypatch, tmp_path, suffix, existing_content):
    plan_path, release_path, _ = gate_fixture(monkeypatch, tmp_path)
    marker = tmp_path / ("normal_test_01" + suffix)
    marker.write_text(existing_content)
    with pytest.raises(ValueError, match="already attempted"):
        runner.release_gate(plan_path, "normal_test_01", release_path)
...
def test_changes_to_frozen_evidence_prevent_start(monkeypatch, tmp_path, changed):
    ...
    with pytest.raises(ValueError, match="[Hh]ash|Implementation changed"):
        runner.release_gate(plan_path, "normal_test_01", release_path)
    assert not (tmp_path / "normal_test_01_session.json").exists()
...
def test_gate_cannot_substitute_another_release_file(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match="release path"):
        runner.release_gate(plan_path, "normal_test_01", other)


def test_second_gate_requires_immediately_previous_session_and_same_protocol_hash(monkeypatch, tmp_path):
    plan_path, release_path, _ = gate_fixture(monkeypatch, tmp_path, "normal_test_02")
    prior = previous()
    prior["protocol_sha256"] = "b" * 64
    session_path = tmp_path / "normal_test_01_session.json"
    session_path.write_text(json.dumps(prior))
    with pytest.raises(ValueError, match="Previous protocol mismatch"):
        runner.release_gate(plan_path, "normal_test_02", release_path)
    prior["protocol_sha256"] = runner.digest(plan_path)
    session_path.write_text(json.dumps(prior))
    _, _, accepted_prior = runner.release_gate(plan_path, "normal_test_02", release_path)
...
def test_main_rejects_unreleased_start_before_any_external_command(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match="Explicit release required"):
        runner.main()
    assert not (tmp_path / "normal_test_01_session.json").exists()
    assert not (tmp_path / "normal_test_01_fan.jsonl").exists()
