...
import pilot_stream_final as live
...
def test_actual_stream_exact_frozen_194_windows_and_three_scores(actual):
    ...
    references,_ = pilot.model_scorers(engine.bundle, BUNDLE)
    for a,b in zip(obtained, expected, strict=True):
        ...
        frozen = pilot.score_window(b['ac_float32'], engine.bundle['scaler'], references, thresholds)
...
def test_live_recorder_keeps_normal_raw_contract_and_requires_declared_load(tmp_path,monkeypatch):
    import pilot_recorder_final as recorder
    import adxl345
